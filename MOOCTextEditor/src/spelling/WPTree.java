...
public class WPTree implements WordPath
{
  ...
  public List<String> findPath(String word1, String word2)
  {
    ...
    Queue<WPTreeNode> toVisit = new LinkedList<WPTreeNode>();

    root = new WPTreeNode(word1, null);
    WPTreeNode currNode = root;
    
    // Only proceed if both word1 and word2 are in the dictionary
    ...
    while (!toVisit.isEmpty())
    {
      ...
      List<String> nearbyWords = nw.distanceOne(currNode.getWord(), true);

      for (String s : nearbyWords)
      {
        if (!visited.contains(s))
        {
          toVisit.add(currNode.addChild(s));
          visited.add(s);
        }
      }
    ...
    // if currNode equals to word2, we have found a path
    List<String> path = new LinkedList<String>();
    if (word2.equals(currNode.getWord()))
    {
